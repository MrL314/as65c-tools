					...
					for cind in range(LINE_LEN):
						...
						if chunk["size"] != None:
							if chunk["type"] == util.DATA_TYPES.VARIABLE:
								...
								if cind > 0 and (LINE[cind-1]["type"] == util.DATA_TYPES.DATA_PAGE or LINE[cind-1]["type"] == util.DATA_TYPES.DATA_BANK):
									LINE[cind]["size"] = 0
									chunk = LINE[cind]								
		...
		for SEC_NUM in range(1, len(sections)):
			...
			for LINE_OBJ in sections[SEC_NUM]["code_data"]:
				...
				while ind < LINE_LEN:
					...
					if LINE[ind]["type"] == util.DATA_TYPES.VALUE:
						use_value = True
						if ind > 0 and (LINE[ind-1]["type"] == util.DATA_TYPES.DATA_PAGE or LINE[ind-1]["type"] == util.DATA_TYPES.DATA_BANK):
							use_value = False
